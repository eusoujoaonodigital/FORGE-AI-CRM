...
import { Plus, Globe, Pencil, Copy, Trash2, Eye, ExternalLink, BarChart3, X } from "lucide-react";
...
const LandingPagesList = () => {
  ...
  const handleDuplicate = async (page: LandingPage) => {
    ...
    const { data, error } = await supabase.from("landing_pages").insert({
      ...
      pixel_meta_id: page.pixel_meta_id,
      pixel_google_id: page.pixel_google_id,
    }).select("id").single();
    if (error) { toast({ title: error.message, variant: "destructive" }); return; }
    ...
    if (sections && sections.length > 0) {
      await supabase.from("landing_page_sections").insert(
        sections.map((s: any) => ({ page_id: data.id, section_type: s.section_type, order: s.order, config: s.config, is_visible: s.is_visible }))
      );
    }
    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-display font-bold">Analytics da <span className="text-gradient-gold">Página</span></h2>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-display font-bold">Landing <span className="text-gradient-gold">Pages</span></h2>
      ...
      {showForm && (
        <div className="glass-card-gold rounded-xl p-5 mb-6 space-y-4">
          <h3 className="font-display font-bold">Nova Landing Page</h3>
          <div className="grid sm:grid-cols-2 gap-3">
            <div>
              <Label>Título</Label>
              <Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Minha Landing Page" className="mt-1 bg-secondary/50 border-border/50" />
            </div>
            <div>
              <Label>Slug (URL)</Label>
              <Input value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, "") })} placeholder="minha-pagina" className="mt-1 bg-secondary/50 border-border/50" />
              <p className="text-xs text-muted-foreground mt-1">/{form.slug || "..."}</p>
            </div>
            <div>
              <Label>Meta Title (SEO)</Label>
              <Input value={form.meta_title} onChange={(e) => setForm({ ...form, meta_title: e.target.value })} placeholder="Título SEO" className="mt-1 bg-secondary/50 border-border/50" />
            </div>
            <div>
              <Label>Meta Description (SEO)</Label>
              <Input value={form.meta_description} onChange={(e) => setForm({ ...form, meta_description: e.target.value })} placeholder="Descrição SEO" className="mt-1 bg-secondary/50 border-border/50" />
            </div>
            <div>
              <Label>Pixel Meta (ID)</Label>
              <Input value={form.pixel_meta_id} onChange={(e) => setForm({ ...form, pixel_meta_id: e.target.value })} placeholder="123456789" className="mt-1 bg-secondary/50 border-border/50" />
            </div>
            <div>
              <Label>Pixel Google (ID)</Label>
              <Input value={form.pixel_google_id} onChange={(e) => setForm({ ...form, pixel_google_id: e.target.value })} placeholder="G-XXXXXXXXXX" className="mt-1 bg-secondary/50 border-border/50" />
            </div>
          </div>
          <Button variant="gold" onClick={handleCreate}>Criar e Editar</Button>
        </div>
      )}
      ...
      {pages.length === 0 ? (
        <div className="glass-card rounded-xl p-12 text-center">
          ...
          <Button variant="gold-outline" size="sm" className="mt-4" onClick={() => setShowForm(true)}><Plus className="w-4 h-4" /> Criar primeira página</Button>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {pages.map((page) => (
            <div key={page.id} className="glass-card rounded-xl p-5 space-y-3">
              ...
              <div className="flex items-center gap-4 text-xs text-muted-foreground">
                <span className={page.is_published ? "text-green-400" : "text-yellow-400"}>{page.is_published ? "Publicada" : "Rascunho"}</span>
                <span>{page._viewCount} acessos</span>
              </div>
